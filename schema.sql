...
create table if not exists stn_client_instance (
    ...
    updated_at timestamptz not null default now(),
    constraint uk_stn_client_instance unique (app_id, client_id, env, region, zone, cluster, namespace_code, group_code),
...
create table if not exists stn_client_snapshot (
    ...
    constraint uk_stn_client_snapshot unique (app_id, client_id, env, region, zone, cluster, namespace_code, group_code)
);
